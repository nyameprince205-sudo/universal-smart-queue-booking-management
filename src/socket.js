...
function initSocket(httpServer) {
  io = new Server(httpServer, {
    // FRONTEND_URL is the same env var password-reset links already use
    // (see auth.controller.js's forgotPassword) — reusing it here means
    // one setting controls both, rather than a second place to remember
    // to update when this app actually goes live. Falls back to the
    // local dev server's own origin so nothing breaks before that
